...
dst_scrape <- function(url,raceInfo){
  ...
  if ("FIS Points Time" %in% colnames(page_tbl)){
    page_tbl$Time <- page_tbl[["FIS Points Time"]]
    page_tbl[["FIS Points Time"]] <- NULL
    page_tbl$Rank <- stringr::str_replace_all(page_tbl[["(Rk.)"]],"[[:punct:]]","")
    page_tbl[["(Rk.)"]] <- NULL
  }
